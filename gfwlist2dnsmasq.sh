...
get_args(){
    ...
    if [ $OUT_TYPE = 'SMARTDNS_RULES' ]; then
        # Check ipset name
        if [ -z $IPSET_NAME ]; then
            WITH_IPSET=0
        else
        ...
        if [ -z $GROUP ]; then
            ...
            GROUP_TEST=$(echo $GROUP | grep -E '^\w+$')
            if [ "$GROUP_TEST" != "$GROUP" ]; then
                _red 'Error: Please enter a valid group name.\n'
                exit 1
            else
                WITH_GROUP=1
            fi
...
process(){
    ...
    if [ $OUT_TYPE = 'DNSMASQ_RULES' ]; then
        ...
        SED_STR='s#(.+)#'
        SED_STR=$SED_STR'server=/\1/'$DNS_IP'\#'$DNS_PORT
        LINE=1
        ...
        SED_STR=$SED_STR'#g'
        sort -u $DOMAIN_FILE | $SED_ERES "$SED_STR" > $CONF_TMP_FILE
